...
### 11. 突破：中间人转发 —— 单连一个设备即获全部能力（最终形态）

思路：出厂模块 F63MAX-1218 原生擅长"启停/暂停/恢复/大屏运行界面"，只缺远程调速；本桥接擅长
远程调速，只缺原生启停。于是让桥接当"中间人"，把两边长处拼起来：
- 对 FitShow/Zwift：本桥接是 FTMS 服务端（SoleF63Max）；
- 对出厂模块 F63MAX-1218：本桥接同时作为 FTMS 客户端，开机即扫描连接它并 Request Control。

FitShow 发来的 Start/Stop（FTMS Control Point 0x2AD9，op 0x07/0x08）→ 转发到出厂模块的
Control Point → 出厂模块用原生流程把机器跑起来/停下（含 `motor_en` GPIO、5秒倒数、大屏进运行
App）。机器进入原生运行态后，FitShow 的速度/坡度再通过 Seewo AIDL 实时生效。

实测全链路（logcat）：

```
op7 → factory CP write 07 → factory indication 0007 → Treadmill started → Set speed result=0（电机跟随）
op8 → factory CP write 0801 → factory indication 000801 → 原生停止
```

✅ 启动 / 停止 / 暂停 / 恢复 / 调速 / 调坡 / 大屏同步 —— **单连 SoleF63Max 一个设备全部可用**。
出厂模块被本桥接占用连接，FitShow 扫描列表里只剩 SoleF63Max（消除"两个设备"困惑）。

> 本节结论**取代**第 8、10 节中"无法软启动 / 两端二选一"的旧结论。
> 代码见 `FtmsBridgeService.java` 的 `startFactoryClient` / `relayStartToFactory` / `relayStopToFactory`。

**最终能力一览：**

| 能力 | 单连 SoleF63Max |
|---|---|
| 启动 / 停止 / 暂停 / 恢复 | ✅（转发出厂模块，原生处理） |
| 大屏进运行 App + 倒数 | ✅ |
| 速度控制（双向） | ✅（Seewo AIDL） |
| 坡度控制（双向） | ✅ |
| 收到 FTMS 指令蜂鸣反馈 | ✅ |
